...
--
-- **順序: この DDL を先に流し、そのあとにデプロイする。**
-- 逆にすると壊れる: settlement-verifier は `SELECT … pu.auth_nonce …` を出し、
-- l1-runner は署名の直後に auth_nonce を UPDATE する。列が無い状態で新しいコードが
-- 動くと、照合 cron と購入 cron の両方が例外で止まる（isMissingSchemaError は
-- 表の不在しか吸収しない）。DDL は既存コードに対して無害——旧いコードは
-- auth_nonce を読まないし、一意 index は現在 0 件の重複にしか触れない。
-- ============================================================
