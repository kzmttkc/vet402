...
import fs from 'fs';
import { viem, ABI, A, W_VET, W_ENS, W_OP2, RPC_S, dns, client, pr, ALL_ROLES, decErr } from '../e15/lib.mjs';
...
res.forEach((c, i) => {
  ...
  else { o.ret = c.returnData?.slice(0, 200); o.logs = (c.logs || []).map(l => EVMAP[l.topics[0]] ?? l.topics[0].slice(0, 10)); }
  rows.push(o);
...
fs.writeFileSync(new URL('../../../out/d_result.json', import.meta.url), JSON.stringify({ block: B.toString(), now: NOW, UR_IMPL, U, P_AG1, tokenId_agent1: TID_A1.toString(), d0, rows, expRows, alRows, totalWriteGas: total }, (k, v) => typeof v === 'bigint' ? v.toString() : v, 1));
