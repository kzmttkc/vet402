...
import fs from 'fs';
import { viem, ABI, A, W_VET, W_ENS, W_OP2, RPC_S, RPC_P, dns, client, pr, ALL_ROLES, decErr } from '../../../lib/common.mjs';
...
fs.writeFileSync(new URL('../../../out/', import.meta.url).pathname + 'hw/h1_d_'+(process.env.RPC==='P'?'pandaops':'sentio')+'.json', JSON.stringify({ block: B.toString(), now: NOW, UR_IMPL, U, P_AG1, tokenId_agent1: TID_A1.toString(), d0, rows, expRows, alRows, totalWriteGas: total }, (k, v) => typeof v === 'bigint' ? v.toString() : v, 1));
