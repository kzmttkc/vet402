import fs from 'fs';
import { viem, ABI, A, W_VET, W_ENS, W_OP, W_OBS, K_ATST, RPC_S, dns, client, pr, ALL_ROLES, ATT_KEY, OBS_KEYS, decErr } from './lib.mjs';
...
fs.writeFileSync(new URL('./k1_result.json', import.meta.url), JSON.stringify({ block: B.toString(), totalGas: total, txCount: rows.length, P_A, P_BC, tokenIds: Object.fromEntries(Object.entries(TID).map(([k,v])=>[k,v.toString()])), rows }, null, 1));
