...
| `check_resource_decision` | Pre-payment decision for one x402 resource — `ALLOW_PAY` / `REFUSE` with the L0–L2 facts that produced it (0.2.0). Takes `amountUsd` / `maxPerTxUsd` / `minL1Deliveries` and returns the server-applied `caller_policy` (2026-09-07) |
