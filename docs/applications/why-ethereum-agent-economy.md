...
- **The catalog is churning.** In the public x402 discovery catalog we track, 19,442 endpoints have appeared; 4,507 are currently delisted; our lifecycle stream has recorded 4,945 delist events and 438 relists. What an agent found yesterday may be gone — or replaced — today.
- **Most of the catalog is not machine-checkable.** Of the tracked endpoints, only 1,355 currently have a machine-verified L0 pass; 18,087 are "unverified" — which means *not machine-checkable*, not dead. Nobody knew this denominator before someone measured it.
...
1. **It buys.** Real settlement with real money, not synthetic probes alone. vet402's L1 level has made 1,681 real purchase attempts across 1,061 endpoints; 697 settled. Each settled purchase is published with its on-chain tx hash.
2. **It publishes failures with the same weight.** The 984 attempts that did not settle are on the same public pages as the successes. A verification layer that only publishes wins is an advertising layer.
...
- **It is a public good in the strict sense.** The observatory's aggregate state is public JSON (`/api/v1/observatory/state`), the methodology is published, daily catalog snapshots are dated (latest: 2026-08-27, 14,935 endpoints fetched), and every settlement claim is independently checkable on-chain. Nobody needs to trust us: they can verify.
