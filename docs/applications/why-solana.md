...
- Solana is the **second-largest chain in the x402 catalog** (after Base at 18,363), and the largest that our real-purchase methodology cannot yet reach.
...
What we ask reviewers to evaluate is not a promise of future traction but a **method that is already running in production on another chain**, applied to Solana: the catalog tracking, daily snapshots (latest: 2026-08-24, 15,274 endpoints fetched), lifecycle events (3,876 delists, 319 relists recorded), the L0→L1 pipeline, and the publish-everything evidence standard are all live and inspectable now.
