...
**vet402 now makes real purchases on Solana — and we say so with the same evidence rule we apply to Base.** Since 2026-08-21 our L1 payer has made **38 real purchase attempts on Solana mainnet**, of which **26 settled and all 26 re-read on-chain as finalized** (genesis hash checked, and the transfer's amount, asset and recipient matched the payment challenge). We are precise about which evidence tier that is: none of the 26 carries the signature-nonce binding we added on 2026-09-04, so they sit in the amount-and-recipient tier, not the strongest one. We publish the tier rather than let the word "verified" do work it has not earned; 3,298 of our 3,336 attempts to date are on Base. Since 2026-09-05 you can check that total yourself without us: `/api/v1/observatory/state` publishes `l1.byChain`, and the Solana row shows 38 attempts with 26 of them settling. The same response splits every settlement into two evidence strengths, so the weaker tier cannot hide inside the headline number.
...
- Solana is the **second-largest chain in the x402 catalog** (after Base at 22,628), and the largest that our real-purchase methodology cannot yet reach.
...
What we ask reviewers to evaluate is not a promise of future traction but a **method that is already running in production on another chain**, applied to Solana: the catalog tracking, daily snapshots (latest: 2026-09-05, 16,158 endpoints fetched), lifecycle events (8,259 delists, 1,176 relists recorded), the L0→L1 pipeline, and the publish-everything evidence standard are all live and inspectable now.
