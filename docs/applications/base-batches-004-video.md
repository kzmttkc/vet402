...
> **応募フォームの実物（2026-08-31 実測）**:
> 「This application requires both a written and video submission and **does not save drafts**.
> Prepare all materials offline first.」——**下書きが保存されない**ので、動画URLと文章を全部揃えてから一度で送る。
> 日程: 8/19 開始 → **9/9 締切** → 9/17 通知 → 9/21〜11/15 プログラム → 11/17 NY デモデイ。
> 制約: 長さ **1–5分**（狙いは **2分50秒**）／提出目標 **9/6**。
...
## 5. 収録の手順（macOS 標準。この Mac を実測: macOS 26.6.2・OBS/Loom なし・QuickTime/iMovie/ffmpeg あり）

**段取りは1コマンドに畳んである**——`zsh scripts/video_setup.sh`（Takeshi_Automation リポ）が
当日の数字を出し、Chrome を 1920×1080 に固定し、台本順に5タブを開く。

1. **⌘⇧5**（スクリーンショットツールバー）を押す。QuickTime の「ファイル > 新規画面収録」も
   今の macOS では**同じツールバーが開く**ので、どちらから入っても同じ
2. **「選択部分を収録」** を選び、Chrome ウィンドウの内側を囲う。
   → **範囲収録にする理由**: 枠の外に台本を置ける（別ウィンドウ・紙・iPhone）。読みながら撮ってよい
3. **「オプション」→ マイクを明示的に選ぶ**（内蔵で可）。「マウスのクリックを表示」ON。
   **ここを忘れると無音の動画ができる**（最頻の失敗。撮り直しになる）
...
```bash
zsh scripts/video_check.sh <撮ったファイル>
```

尺（60〜300秒）・高さ（720以上）・**音声トラックの有無**を判定し、音割れとサイズを警告し、
トリムと 1080p 変換のコマンドを実ファイル名で出す。わざと不合格な動画（6秒・360p・無音）で
自己テスト済み——3項目とも FAIL を返すことを確認してある。
残りの目視確認は2つだけ: 画面に個人情報・APIキー・通知が映っていないこと、
**読んだ数字が当日の出力と一致**していること。
