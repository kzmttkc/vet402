...
- Public x402 catalog tracking: 19,023 endpoints (15,251 active); daily snapshots (latest 2026-08-25, 15,239 fetched)
- L0 machine verification: 1,246 published pass
- L1 real purchases: 1,596 attempts / 653 settled, every settlement published with its Base tx hash; non-settles published with the same weight
- Lifecycle event stream: 4,135 delists, 363 relists, 3 settle-drops
...
*Pre-grant figures retrieved from /api/v1/observatory/state on 2026-08-26. Refresh before each submission: `python3 scripts/grant-figures.py --check`.*
