...
| L0 machine-verified pass (probe, no purchase) | 11,932 published; 7-day coverage 77.6% of active endpoints |
| L1 real purchases | 3,336 attempts across 2,302 endpoints — 1,669 settled, each with its tx hash; the non-settles published with the same weight. **Base 3,298 / Solana 38** (`l1.byChain`), and every settlement is split into two evidence strengths: `settledNonceBound` 107, `settledAmountPayeeOnly` 1,562 |
...
| **Solana in the catalog** | **308 endpoints tracked (231 active) — second-largest chain after Base (22,628)** |
...
**Honest status (corrected 2026-09-05): the Solana payer is live, and we are not asking you to fund what already ships.** We enabled it in production on 2026-09-04, and it has been buying since 2026-08-21: **38 real purchase attempts on Solana mainnet, 26 settled, all 26 re-read on-chain as finalized** (genesis hash checked; amount, asset and recipient matched the challenge). Evidence tier, stated rather than glossed: none of the 26 carries the signature-nonce binding introduced on 2026-09-04 — they are the amount-and-recipient tier. This proposal previously described the payer as written-but-undeployed; that was true when it was drafted on 2026-08-20 and is no longer true. What is *not* done is the part money actually buys: **coverage** (31 of 323 in-cap Solana endpoints have ever been bought from) and **coverage alone** — legibility landed while this was being written: as of **2026-09-05** the public `/api/v1/observatory/state` exposes `l1.byChain` (Base and Solana attempts/settled separately) and the two evidence tiers, so a reviewer can now total our Solana settlements from the aggregate API without trusting us).
...
|---|---|---|---|---|---|
| M1 | ~~Solana settlement live~~ **delivered before this proposal (2026-08-21 / production-enabled 2026-09-04)** — restated here as evidence, not as work to fund. Remaining M1 scope **also delivered 2026-09-05**: `l1.byChain` now publishes per-chain attempts/settled, so the acceptance check below is satisfiable today (`chain: "Solana"`, `settled: 26`) | SVM exact-scheme payer enabled in production; first real Solana settlements published; history/state attribution fixed to settlement-chain (§2.1) | `curl -sL https://vet402.com/api/v1/observatory/state` shows a Solana `byChain` entry with L1 fields and `settled > 0`; a published purchase page shows a **base58 transaction signature** (not a 0x hash) resolvable on Solscan/Solana Explorer | 2 weeks | $0.00 (no purchase capital needed; the code exists) |
