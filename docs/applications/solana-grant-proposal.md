...
**Honest status (corrected 2026-09-05): the Solana payer is live, and we are not asking you to fund what already ships.** We enabled it in production on 2026-09-04, and it has been buying since 2026-08-21: **38 real purchase attempts on Solana mainnet, 26 settled, all 26 re-read on-chain as finalized** (genesis hash checked; amount, asset and recipient matched the challenge). Evidence tier, stated rather than glossed: none of the 26 carries the signature-nonce binding introduced on 2026-09-04 — they are the amount-and-recipient tier. This proposal previously described the payer as written-but-undeployed; that was true when it was drafted on 2026-08-20 and is no longer true. What is *not* done is the part money actually buys: **coverage** (31 of 323 in-cap Solana endpoints have ever been bought from) and **legibility** (as of 2026-09-05 the public `/api/v1/observatory/state` exposes no per-chain L1 counts — `l1` carries only the totals — so these settlements are visible on each endpoint's record page but a reviewer cannot total them from the aggregate API. Per-chain counts and evidence tiers are being added; this proposal does not assume them).
