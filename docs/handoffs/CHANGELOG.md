...
## 2026-09-19 15:10 JST — Late-settlement recovery covers `delivered_no_receipt` / `settle_claimed_unverifiable`, XRPL recovery binds on the signed blob hash, and an unfunded XRPL payer closes the lane (vet402.com core; not pushed by the author — independent review first)

- **What changed**: (1) `src/lib/settlements/recover-late.ts`: the target statuses are now `LATE_RECOVERABLE_STATUSES = settle_failed | delivered_no_receipt | settle_claimed_unverifiable` (was `settle_failed` only). The join is unchanged (same chain, payer, pay_to, exact amount, −2 min / +30 min window, tx not claimed by any other purchase, one tx → one purchase). `settle_claimed_unverifiable` rows carry the seller's malformed id in `tx_hash`; it is replaced by the indexed tx and kept in `raw_response_meta.lateSettlement.replacedTxHash` (it was already in `raw_settlement`). `lateSettlement.priorStatus` and the correction-log `before` now carry the real prior status / tx_hash instead of a hard-coded `settle_failed` / `null`. Linked rows still go to `settle_claimed` (never `settled`); the verifier decides. (2) Same file, XRPL only: a row on `xrpl:0` is linked only when `upper(settlements.tx_hash) = upper(auth_nonce)` — the signed blob hash the runner stores before submitting. An XRPL row without `auth_nonce` is not linked. (3) `src/lib/observatory/l1-runner.ts`: when the funds gate answers `payer_unfunded` for the XRPL payer (RLUSD insufficient, or unreadable — which includes `xrpl_fee_unfunded`), the batch sets `xrplLaneUnavailable` on the first occurrence and `summary.xrplLaneClosed = "payer_unfunded"`. `purchaseOne` returns `payerChain` with the `payer_unfunded` outcome so the loop can tell which chain ran dry.
- **Why**: (1) A signed EIP-3009 / XRPL Payment stays live whatever the seller answered. Rows that ended as `delivered_no_receipt` (200, no receipt header) or `settle_claimed_unverifiable` (receipt with a malformed id) could be settled on-chain with no way to reconcile them, exactly like the 2026-09-04 `settle_failed` case. (2) There was no pg test of index-xrpl → recover-late. Writing it showed a real mislink: with two unsettled XRPL rows to the same payee for the same amount inside the window, the tx was attached to the earlier-attempted row even though its `auth_nonce` was a different hash; the verifier would then refute that row with `nonce_not_used` — our mislink recorded as a finding about the seller — and the true row could never claim the tx. XRPL gives us the exact binding (tx hash == signed blob hash), so recovery uses it. (3) With XRPL still preferred after a `payer_unfunded`, every remaining lane candidate in the batch hit the same gate: no row, not bought on Base either, so Base-first sellers in the XRPL lane went unmeasured on any rail until the wallet was refilled.
- **Impact**: Spending caps, `reserveSpend`, the funds gate itself, `/decision` `facts.l1` keys, `l2_*`, `/tokyo` are untouched. No new public status or correction reason (`settlement_backfill` as before). More rows can now move `delivered_no_receipt` / `settle_claimed_unverifiable` → `settle_claimed` → `settled` (or `settle_claim_refuted` if the verifier's nonce binding says the tx is not ours — same exposure the `settle_failed` path has had since 2026-09-04 on EVM, where the index has no nonce to pre-bind on). XRPL recovery is strictly narrower than before. After an XRPL `payer_unfunded`, Base-first lane candidates are bought on Base in the same batch, XRPL-primary rows are skipped without a request, and `summary.payerUnfunded` counts 1 for XRPL instead of one per candidate; other chains' funds gates keep the old keep-walking behaviour. `xrplLaneClosed` gains the value `"payer_unfunded"` (cron response / logs only).
- **Tests**: `tests/late-settlement-recovery.pg.test.ts` (+6: both new statuses recovered; amount / payee / payer / window mismatches not linked; tx already claimed not linked; one tx → one row across statuses; refuted / unsigned statuses untouched), `tests/late-settlement-recovery-xrpl.pg.test.ts` (new, 7: real `indexXrpl` wiring with a faked `account_tx`; positive on hash match for `settle_failed` and `delivered_no_receipt`; negatives for hash / amount / destination mismatch; two-row case links the hash-matching row, not the earlier one; no `auth_nonce` → not linked), `tests/l1-xrpl-secondary-accept.pg.test.ts` (+2: insufficient and unreadable XRPL balance close the lane, later Base-first candidates bought on Base, zero XRPL rows), `tests/l1-xrpl-lane.pg.test.ts` (XRPL-primary: 1 unfunded, lane closed, no further XRPL requests). Each new positive/lane test was seen red before the fix.
