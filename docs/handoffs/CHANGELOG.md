...
- **影響**: Base・Arc・Tempo・Solana の経路は不変（XRPL のフラグ OFF／`XRPL_RPC_URL` 無しなら枠は 0 行で、同じ行は従来どおり Base で買われる）。`/decision` の facts.l1 のキー・公開語彙は不変。XRPL で 1 度 settle_claimed 以上（または非決済）になった endpoint は次から Base に戻る（レーンの実績は 1 endpoint 1 回）。1 バッチ 1 件なので XRPL の掃引は 1 cron 1 件ずつ進む
