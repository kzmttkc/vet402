...
- **何を変えたか**:
  ...
  - (2) `l1-runner.ts`: Tempo（MPP）の `createMppCredential` が throw したら、`resolveReservationAsFailed`（settle_failed・`spent_units` 据え置き）へ落とさず、新しい `releaseReservation` で予約行を**消す**（`status='in_flight' AND tx_hash IS NULL` の行だけ）。outcome は `skipped`・`spent: 0n`。
