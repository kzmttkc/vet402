...
- **テスト**: `tests/late-settlement-recovery.pg.test.ts`（新 status の回収・額／宛先／払い元／窓の不一致・使用済み tx・候補 2 行の tx はどちらにも貼らない（status またぎも）・候補 1 行の tx だけ貼る・`rejectedTxHashes` の除外と持ち越し・`settlement_verified*` の NULL 戻し・対象外 status）、`tests/settlement-verifier-late-link.pg.test.ts`（新規: 3 つの元 status それぞれで refuted にならない・`hooks.l1` が呼ばれない・status と tx_hash が戻る・同じ tx を二度拾わない／`priorStatus` の無い旧い行／回帰: 売り手が名指した tx の nonce 不一致は refuted + `hooks.l1`／late-link でも nonce が合えば settled）、`tests/late-settlement-recovery-xrpl.pg.test.ts`（新規: 本番配線の `indexXrpl` + 偽の `account_tx`。正例 2・hash／額／宛先の不一致・2 行競合は hash の合う行・`xrpl:1` も束縛・`auth_nonce` 無し）、`tests/l1-xrpl-secondary-accept.pg.test.ts` / `tests/l1-xrpl-lane.pg.test.ts`（資金切れでレーンを閉じる）。どれも修正前に赤を確認。
