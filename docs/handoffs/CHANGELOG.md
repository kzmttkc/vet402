...
## 2026-09-19 JST — Arc のレーン: カタログが Arc の accept を「先頭と別の payTo」で宣言している行を Arc で買えるようにした（ブランチ `fix/arc-declared-payto`・未 push・独立レビュー待ち）
...
## 2026-09-19 07:23 JST — XRPL の有料要求が `invalid_payload` で断られていた原因: 封筒の `payload.invoiceId` が無かった（ブランチ `fix/xrpl-invalid-payload`・未 push・独立レビュー待ち）
