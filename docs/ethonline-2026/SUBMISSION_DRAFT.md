...
Tag pre-ethonline-2026 = commit c42daca, cut 2026-09-04 00:05:36 UTC, five minutes after the window opened, and pushed. Everything we claim is after it. main is also this product's production branch, so the full range contains work we are not claiming; both commands are in README.md:
...
| `{{mutations}}` | `<!-- n:sdk_mutations -->`（`npm run refresh-numbers`） | 動く（09-07 記録値 27） |
...
| `{{sdk_tests}}` / `{{mcp_tests}}` / `{{demo_tests}}` | `VIDEO_SCRIPT.md` §5 のコマンド（09-07: 178 / 65 / 65） | **動く** |
...
| `{{sdk_tests}}` | `cd packages/sdk && npm ci && npm test 2>&1 \| grep -E '^ℹ (tests\|fail)'` | 1609 / fail 0【`refresh-numbers.json` 09-07】 | I |
| `{{mcp_tests}}` | `cd packages/mcp-server && npm ci && npm run build && npm test 2>&1 \| grep -E '^ℹ (tests\|fail)'` | 748 / fail 0【`refresh-numbers.json` 09-07】 | I |
