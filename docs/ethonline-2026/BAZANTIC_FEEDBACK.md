...
5. **The "JWT that bypasses x402/MPP for testing"** mentioned in `#partner-bazantic` — on 2026-09-06 we could not find it in the docs or the dashboard. Answered 2026-09-09 by Tom Hay (Bazantic) on Discord: a JWT created at `bazantic.com/api-keys` can be used as an API key for the gateway to bypass the 402 challenge, so no transaction has to be signed. We read the CLI's `--auth-type api-key | jwt | x402-mpp | basic` (`@bazantic/cli@0.8.0`, default `x402-mpp`) as the gateway owner's upstream auth setting for the registered endpoint; sent as a buyer-side request header (`Authorization: Bearer`, `x-api-key`) it made no difference in our 2026-09-09 re-measurement, which fits that reading. Our gateway stays on the default `x402-mpp`. A line about it next to the pricing docs would make it easy to find.
