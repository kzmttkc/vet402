...
1. **On 2026-09-06, $0 routes answered 402.** All 57 routes are at 0 mcents, yet every unpaid call got a 402. The docs say only `tools/list` and price discovery are free; the body comes after payment. With `baz curl … --max-amount 0` (and with my own signer) the REST route returned 200 — after the facilitator posted a **0-USDC transfer on chain**. Re-measured 2026-09-09 08:30–08:40 JST: the same Gateway's 0-mcent tools (`info`, `getHealth`, `resolveQuery`, `getPayeeScore`) returned their body on an unpaid MCP `tools/call` — no 402, no settlement. On 2026-09-11 an unpaid `tools/call` to 12 of the tools v1 used got no 402.
2. **On 2026-09-06, MCP `tools/call` could not pay.** A `PAYMENT-SIGNATURE` header on the `/mcp` POST was ignored, and the tool result was `isError: true` with the 402 body. `initialize.instructions` was null and no tool schema had a payment field.
3. **So on 2026-09-06 a standard MCP client could not use any of the tools.** "Add to Claude / Cursor / ChatGPT" connected, listed 57 tools, and every call returned 402 — including the 37 tools described as needing no key. Run `093254Z` measured exactly that: 0/10 and 0/10. Re-measured 2026-09-09 08:30–08:40 JST: a stock MCP client could call the free tools and got the body; `093254Z` records the 2026-09-06 state.
...
6. **`baz recipe`** (`baz recipe install`, the documented way to run a public Recipe with a payer) was not in `@bazantic/cli@0.8.0`, the npm latest when I checked on 2026-09-06; the docs ran ahead of the CLI. Versions 0.9.0, 0.10.0 and 0.10.1 were published on 2026-09-11 and 2026-09-12 (UTC). `@bazantic/cli@0.10.0` carries `baz recipe`: its `--help` (run 2026-09-13) lists `recipe list`, `get`, `create`, `update`, `publish`, `unpublish`, `delete`, `install` and `mcp`, so the docs and the CLI now agree.
...
- Ship `baz recipe` in the CLI, or remove it from the docs until it ships. (Shipped: `@bazantic/cli@0.10.0` carries it; see §4 #6.)
...
**Differences from v1 that are not the Recipe.** No payer key was passed, so the bridge was never built: **102 tool calls, 0 settled, 0 transactions** (v1: 110 calls, 88 transactions of 0 USDC). The Gateway's 0-mcent tools answered an unpaid `tools/call` with the body (12 of the tools v1 used: 0 × 402 right before and right after the run). The Recipe on bazantic.com was v2 for the run; the planned v2.1 (one line about `l1_inconclusive`) was not published — a recorded deviation.
...
- "F2 is not fixed in either condition" — **right.** `/decision` answered 404 `not_found` for The Graph's resource when I checked on 2026-09-11; both conditions scored 0/3.
