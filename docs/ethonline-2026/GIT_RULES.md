...
## 会期中のコミット件名は英語（発効 2026-09-08 20:00 JST・CEO 決定）
...
1. **発効は 2026-09-08 20:00 JST。それ以降に作るコミットの件名を英語で書く。** 本文（body）は
   日本語のままでよい。接頭辞（`ethonline:` / `feat` / `fix` / `docs`）は変えない。
   例: `ethonline: fix(skill): fold the MCP tools/call JSON onto one line — stdio drops folded requests`

   **「9/8 から英語」ではない。** 決定はこの日の 20:00 であって、日付の頭ではない。
   実測（2026-09-08 20:44 JST・`origin/main`）: 同日 00:00 以降の 43 本は**日本語 38 / 英語 5**。
   20:00 以降の 6 本でさえ **4 本が日本語**（決定より前に走り出していたエージェントの分）。
   規則が掛かるのは**これから作る件名**であって、規則を書いた時点の履歴ではない。
   **達成済みの事実として書かない**——`evidence` を売る製品が自リポの主張で外していれば、
   審査員はまずそこを突く。数え直す:
   ```bash
   TZ=Asia/Tokyo git log origin/main --since='2026-09-08 20:00:00 +0900' --pretty=%s \
     | grep -cP '[\x{3040}-\x{30ff}\x{4e00}-\x{9fff}]'
   ```
2. **過去の件名は書き換えない。** 履歴の改変は禁止（`rebase -i` も `filter-branch` もしない）。
   発効前が日本語なのは事実として残す。README の *Language* 行が書けるのは
   **「規則がいつ発効したか」までで、「以後すべて英語である」ではない。**
