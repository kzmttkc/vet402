...
- **`"both"` は「両方読めなければ拒否」**（強い fail-closed）。**2026-09-05 訂正**: ここは元々
  「片方だけ読めたときは、どちらで判定したかを決定行に明記する」と書いており、拒否を要求していない
  ようにも読めたが、**テスト C12 は `refused` を要求している**。テストが仕様なので拒否で実装した。
  どちらが読めなかったかは理由コードに残る（`subgraph_evidence_unavailable` 等）。
- **床は源ごとに当てる。源をまたいで件数を足さない**（テスト D16）。
- **宣言した床は、宣言した `source` が評価できなければならない。** `{source:"vet402", minSubgraphReceipts:…}`
  のような組み合わせは**呼び出し側エラー**（`invalid_evidence_policy`）で、通信の前に throw する。
  **黙って無視しない**（2026-09-05 まで、まさに黙って無視されていた）。
- MCP `pay_if_trusted` から同じ選択ができるようにする（賞の要件「再利用可能な道具」に当たる）。
  **2026-09-05 時点では未実装**——MCP は証拠源を選べない。残りの3面（OpenAPI / `/docs/api` / MCP スキーマ）と同じ扱い。

**前提**: Graph Gateway の API キー（無料枠・月10万クエリ）。賞は「モック・ローカルのみ・
静的データは不可」と明記しているので、キーが無ければこの枠は成立しない。
**キーは SDK に埋め込まず、呼び手が `policy.evidence.graphApiKey` で渡す**（2026-09-05 決定）。
理由: 主張は「`source:"subgraph"` なら我々の台帳の件数を1件も使わない」であり、
**呼び手が自分の鍵で自分で引いて初めて成立する**。我々の鍵を通せば、結局 vet402 を信じている。
落とし穴は `WINDOW_PLAN.md` §15（鍵無しは 403 でなく 200＋GraphQL errors・role の絞り・UA）。
