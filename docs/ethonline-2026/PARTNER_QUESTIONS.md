...
> **【2026-09-05 20:4x 執行部・この文書の質問は全部やらなくてよくなった】**
> Takeshi の手番に 5 日間置いていたが、**3 件とも実測で不要になっていた**。手番から外す。
>
> - **(A) The Graph** … 聞くはずだった 2 問は、ハッカソン戦略が**自分で測って答えを出した**。
>   `GRAPH_EVIDENCE.md`（9/3）で分散ネットワーク上の `x402 Base` subgraph を特定し
>   （ID `Cb56epg…`・block 50,799,813・`hasIndexingErrors:false`）、生データでフィクスチャ2件を照合。
>   さらに **9/5 に The Graph へ $0.01 を実際に払った**（`73ef91a`）。
>   「Gateway 経由で足りるか」「どの subgraph か」は、聞くより先に**やって確かめてある**
> - **(B) World** … **2026-09-03 に World 枠を切った**（`OWNER_NOW.md`）。宛先ごと消滅
> - **(C) Hedera** … 「聞く相手は Hedera ではなく World だった」と判明し、その World を切ったので消滅
>
> **残す理由**: 文面そのものは使い回せる（次の大会で continuity 枠を尋ねるとき）。
> だが **今この文面を Discord に貼る必要は無い**。貼ると、既に自分で解いた問いを
> パートナーに聞くことになる。
