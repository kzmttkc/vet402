...
**事後の探索指標（採点には使わない・事前登録に無い・生ログから再計算可）**: 挙げた理由コードのうち vet402 の語彙（`vocabulary.ts`＋SDK の理由コード 45 語）に**実在する**割合——**A 6%（2/32）／B 31%（10/32）**。全コードが語彙内の試行: A 0/10・B 2/10。**Recipe は「語彙」を直し、「判定」は直さない。**
