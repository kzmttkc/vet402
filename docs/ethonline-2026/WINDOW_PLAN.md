...
**事後の探索指標（採点には使わない・事前登録に無い・生ログから再計算可）**: 挙げた理由コードのうち vet402 の**実在する語**（oracle が実際に返した階層コード `l0_pass`/`l1_delivered`/`l2_undeclared` を含む）の割合——**A 59%（19/32）／B 91%（29/32）**。全コードが実在の試行: A 7/10・B 7/10。
（訂正 09-07 06:4x: 最初に書いた「A 6%／B 31%」は `vocabulary.ts`＋SDK の語だけで数え、oracle が返す階層コードを落としていた過小評価。方向は同じ。出所: `npm run metrics -- docs/ethonline-2026/ab/2026-09-06T213134Z` の「Exploratory: vocabulary rate」が集合 (i)(ii) を両方印字する。**同出力は A を (ii) 20/32・(i) 3/32 と数える**——採点と同じ trim＋小文字化で数えるため、A の `Unverified` 1 件が語彙の `unverified` に当たる。上の 19/32・6% は大小を区別した手数え）
