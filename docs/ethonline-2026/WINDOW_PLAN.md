...
| 9 | **`/api/health` が `feedback_stats_unavailable(deadline:getLogsChunked)` で degraded になる**（09-10 02:59–04:00 JST の **11 行**・本番 `health_snapshots` 直読み・`latency_ms` 1,899–2,821・11 行とも `payee=ok`）。経路は `src/lib/chain/erc8004.ts:222` の `TAIL_SCAN_DEADLINE_MS = 2_500`＝tail 走査の**内側**予算で、#6 の `wallet_metrics`（外側 3,500ms・買い手側ウォレット指標）とは**別の脚**。過去 7 日の UTC 時刻別分布は `utc16 deg=7 ok=36` ／ `utc18 deg=11 ok=37` ／ `utc19 deg=7 ok=47` ／ `utc23 deg=5 ok=41` で、残る 20 時間帯は `deg=0〜3`——**時刻で偏り、デプロイでは偏らない**。影響範囲は `src/lib/scoring/engine.ts:327` から届く**エージェントスコア面だけ**（`/agent/[agentId]`・badge・passport・`v1/scores/batch` ほかで `src/lib/scoring/verdict.ts:75` risk="high"・`src/lib/scoring/helpers.ts:364` −15）。対策候補（内側予算の見直し／`getIndexerPublicClient()` の上流の複数化／chunk 幅）は**この行では決めない** | 判定は変わらない（`/api/health` が degraded を返すだけで、SDK が叩く `/resources/{id}/decision`・`/payees/{addr}/score` は DB 由来の別の脚を読む＝到達 0 件）。上流の RPC も時間予算も #5・#6 と同じ理由で会期中に触らない（`eth_getLogs` を拒む提供元がある——`src/lib/chain/client.ts:69`）。**本番の RPC 実値は【未確認】**なので「公開 RPC が原因」とは書かない |
