...
言う文の語数（2026-09-11 に数えた）: 0:00 は 50 語、0:30 は 90 語、1:30 は 77 語、2:30 は 68 語、3:15 は 106 語、締めは 29 語。150 語/分で 0:30 は約 36 秒、1:30 は約 31 秒、2:30 は約 27 秒で、網の待ち（各 3〜5 秒）を足しても枠に収まる。**3:15 の段は 106 語・約 42 秒**。画面は 0.3 秒で出るので待ちは無いが、45 秒の枠の余りは約 3 秒しかない。**締めの 29 語（約 12 秒）はその余りに入らない**ので、3:15 の段を全部言うと締めが 4:00 を約 9 秒超える。**4:00 で止められる前提**で 3:15 の段は削れる作りにしてある。
...
A: *Nothing changes: BLOCK still refuses. `requireVet402Allow: false` waives exactly one thing — a WARN — and only when every declared floor is met. BLOCK is not vet402's opinion. It is an operator-level global block. `degraded`, meaning vet402 could not read an input, also stays a refusal. Both are pinned by tests in the SDK and through the MCP bridge. On September 11 the SDK had 45 mutations, and four of them flip exactly that boundary and turn the suite red.*
要旨: 免除は WARN だけ。BLOCK と degraded は常に拒否。SDK と MCP の両方でテスト固定。
証拠: `WINDOW_PLAN.md` §3.2.1 の表／`packages/sdk/test-mutations.mjs` M01〜M04（M01・M02 が BLOCK、M03・M04 が degraded の免除境界。**45 本は 2026-09-11 の実走 `all 45 mutations killed`**。A/B 側の 27 本とは別の集合で、09-11 まで台詞がそちらの本数を言っていた）／`SKILL.md` "`pay_if_trusted` with The Graph evidence"（H1–H7）。
...
A: *It found that green tests were lying. Tests that only looked at `status` and signer calls stayed green when the whole ALLOW gate was removed — the run refused for a different reason. Switching the payment module to a static import turned no test red until I added the `dist` module-graph test. The mutation script breaks one gate at a time — BLOCK waiver, floor comparison, `payTo` check, ceiling, nonce retention, `_meta.block` — rebuilds, and requires red. In the submitted SDK, <!-- n:sdk_mutations -->45<!-- /n --> mutations, all killed. Four survived on September 6 and became tests.*
...
聞かれたら *"I wrote that the next bad row would name the path, and it did: eleven rows on September 10 all carried `deadline:getLogsChunked`, the tail scan over the feedback logs. That name covers two cases, the scan running out of its time budget or a rate-limit wait that does not fit in the time left, and as of September 11 I have not pinned down which one it is. Failing closed there moves only the agent-score surface, because the gate's two faces read a different leg. I wrote the cause down as a post-window item, and I will fix it after judging."*

**Q18. Are the numbers in the video still the same today?**
A: *No, and they should not be. The video is a recording. Its numbers are the record of the day it was shot. In the video, The Graph's subgraph shows four hundred and eighty-three receipts for that wallet, read on September 8. The count only grows from there, and the payee score moves too. That is exactly why every evidence row carries `_meta.block.number`, `deployment` and `queriedAt`: you can tell when a number was read, and whether it was read at all. Anything I say live, I read off the screen in front of you.*
要旨: 動画は撮影日の記録。提出動画 v4.2 の画面は 09-08 の 483 件。件数は増える一方で、スコアも動く。**だから**決定行に block・deployment・時刻がある。生で言う数字は画面から読む。
証拠: 提出動画 v4.2 の S2 `totalPayments 483`（09-08 09:0x の実測・`VIDEO_SCRIPT.md` §「2026-09-08 の規律変更」）／§0「受取人スコアの値を口で固定しない」／§6「当日取り直す」（09-07 12:xx 427・09-10 と 09-11 は 512）／09-08 22:0x 実測では**同じ問いに違う値**が返った（画面で読む）。§7 の 01/02/08 に当日の値が採ってある。

**Q19. Your Bazantic numbers from September 6 and September 9 disagree. Which is right?**
A: *Both, on their dates. The Gateway changed, and the harness did not. On September 6 every unpaid call to a zero-price tool got a 402, and reading a free tool cost a settlement: eighty-eight free reads, eighty-eight zero-USDC transactions on chain. On September 9 I measured the same Gateway again, and the zero-price tools returned their body on an unpaid MCP call. No 402, no settlement, and the bridge was never entered. I kept both sets of numbers with their dates. I do not claim to know why it changed.*
...
| `n:demo_tests` | <!-- n:demo_tests -->178<!-- /n --> | `npm test --prefix examples/ethonline-2026-demo 2>&1 \| sed -n 's/^ℹ tests //p'`（鍵不要・0.5s） | 画面のみ（言わない） |
...
| 想定質問 15 件 | **18 件**（Q16 コミットの言語 / Q17 `/status` の error / Q18 動画の数字） | 審査基準に *"Proper use of git commit history"* が明記されており、§4.5 が `/status` を「見せてよい 2 本」に挙げているのに、どちらも答えが無かった。提出動画 v4.2 の receipts は 09-08 の 483 で、09-10 には 512 に増えていた（§6） |
