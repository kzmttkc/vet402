...
|---|---|---|---|
| A1 | **2026-08-25 訂正。アカウントが2つある。** 参加が確定しているのは **kazumototakeshi@gmail.com** の方で、ETHGlobal から 2026-08-20 に "You've confirmed your spot for ETHOnline 2026" が届き、My events には ETHOnline 2026 / ETHGlobal Tokyo 2026 / Pragma Tokyo 2026 の3件が並んでいる。一方 08-23 に私が Hacker 申請を出したのは **kzmttkc314@gmail.com**（GitHub @kzmttkc・X @Takeshi_Web5・Discord sen_web3 が紐付く方）で、**Continuity トラックの選択はそちらに入っている**。提出は確定済みアカウントで行う以上、**そちらの ETHOnline ダッシュボードで Continuity (Hack on Existing Project) が選択されているかを確認する**のが本当の A1。未選択なら選ぶ（フォームに欄が無ければ hello@ethglobal.com）。重複申請は取り下げる。 | Human | 確定済みアカウント側で Continuity が選択済みと確認できる |
| A1.1 | **2026-08-25 追加確認（Takeshi）: `kzmttkc314` 側にはウォレットが1つも紐付いていない。賞金・ステーク返却・過去参加を含め、実体は全て `kazumototakeshi` 側にある。** よって提出アカウントは `kazumototakeshi` で確定。08-23 に私が出した申請は 08-20 の参加確定より後の重複で、受け取り口も無く使えない——取り下げる。残る確認は1つ、**提出アカウント側で GitHub（`kzmttkc/vet402` のオーナー）を連携できるか**。同じ GitHub を2アカウントに繋げない仕様なら、先に `kzmttkc314` 側の連携を外す必要がある。 | Human | 提出アカウントで Continuity が選択済み・提出画面でリポを指定できる |
| A2 | ~~Tokyo Continuity apply~~ **Tokyo 2026 は登録済み**（My events に SEP 25–27 で在る。Pragma Tokyo 9/26 も）。残るのは Tokyo 側の Continuity 選択の確認（締切 09-23）。 | Human | Tokyo ダッシュボードで Continuity 選択を確認 |
