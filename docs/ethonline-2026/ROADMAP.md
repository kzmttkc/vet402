...
| A1.1 | 提出リポ `kzmttkc/vet402` の GitHub は kzmttkc314 側に連携済み。**確定済みアカウント側の GitHub 連携**を確認する（同じ GitHub を2アカウントに繋げない場合、提出フォームでリポを指定できるかを先に見る）。 | Human | 提出画面でリポを指定できることを確認 |
