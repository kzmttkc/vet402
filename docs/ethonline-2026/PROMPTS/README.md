...
- 中身: **実際に投げた指示文をそのまま**。要約しない。言い回し・句読点・改行は変えない
- ローカルの絶対パス（`/Users/<name>/…`）は `<local>` へ置換してよい（2026-09-09 監査・所見 5）
- 固有名・識別子・数字の明らかな誤記は直し、直した語の直後に `〔原文: wet402〕` のように元の表記を添える。黙って直さない。迷うものは直さない（2026-09-11）
