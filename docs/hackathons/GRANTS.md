...
**グラント側の裁定（2026-08-31・受け取って即日）**: **応募しない**。2026-08-23 の取り下げ判断を維持する（§3 P3）。

- **訂正**: 取り下げの理由③に「Demo Day が Devcon(11/3–5)・Mumbai(11/6) と重なる」と書いたのは**誤り**。
  申し送りの一次確認どおり **デモデイは 11/17** で重ならない。ただしプログラム期間 **9/21–11/15** は
  Tokyo(9/25–27)・Devcon・Mumbai を丸ごと含む。
- **維持する理由（こちらが本体）**: ①$100K は出資であり受入れが参加条件。**測っているカタログを持つ側の
  ファンドが株主になる**利益相反だけは、証拠で反論できない——vet402 の唯一の資産がそこなので売らない。
  ②中立ゆえに測る相手へ売る物が無く、投資家トラックの収益物語は中立を曲げないと書けない。
- **締切 9/9 まで、オーナーの一言で覆せる**。覆る場合、残るのは撮影のみ（素材は下記のとおり完成済み）。

**素材は捨てない**——別用途で使う:
- `vet402_video_numbers.py` は「当日の数字＋その日のレシート/Basescan URL」を一次データから出す。
  これは **`/demo` の録り直し**（Base Builder Grants の1分デモ・2026-08-25 公開）にそのまま使える。
- 6ショット台本と禁止リスト（実装より強い主張をしない）は、今後の動画すべての土台にする。
- `video_check.sh` の機械判定（尺・解像度・音声トラック）は、提出物を持つ他の応募でも流用する。
