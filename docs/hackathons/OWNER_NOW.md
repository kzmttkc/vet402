...
## 09-11〜09-12（先に押さえておく・ハッカソン戦略の持ち物）

9. **デモ動画のナレーションを録る（15分・AI 音声は自動却下＝失格側）**。
   **09-11 に、そのまま読める一文ずつの台本を渡します。** 要るのは静かな部屋とマイク1本だけ。
   画面収録と編集はこちらでやります。読み上げ 3〜4 分 × 2〜3 テイク。
   **英語を推奨**しますが判断はお任せします（日本語なら英字幕を付けます）。
   **この日だけは 15 分ください。** 提出は 09-13 午前なので、遅れると詰みます。
10. **提出ボタンを押す（09-13 午前・数分）**。エージェントは ETHGlobal の画面を押せません。
