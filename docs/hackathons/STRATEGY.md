...
| 何のために大会に出るのか（全大会共通） | **§0.5**（2026-09-12 オーナー確定。1) vet402 の GTM 2) パートナー賞 3) 総合。パートナー賞コメントの型もここ） |


---

## 0.5 全大会共通 — 目的の順位とパートナー賞の型（2026-09-12 オーナー確定）

この節は **ETHOnline に閉じない**。ETHGlobal Tokyo（2026-09-25〜09-27）、ETHGlobal Mumbai（2026-11-06〜11-08）、
およびその先の大会すべてに同じ順序で効く。§5.1 の「総合優勝は狙わない」は、この節の 0.5.1 が理由である。

### 0.5.1 目的の順位（この順で資源を割く）

1. **vet402 の GTM** — 認知・信頼・被参照。ハッカソンはマーケティング枠であって賞金事業ではない
2. **パートナー賞（枝葉）の確実な獲得** — 1件ずつ確実に拾う
3. **総合（大賞・ライブ審査）** — 上の2つを損なわない範囲で取りに行く。ここに時間を先に使わない

**オーナーの言葉（2026-09-12・原文のまま）:**
...
（**2026-09-12 に curl で取得して原文照合。HTTP 200・127,963 bytes**。§1.4 の要約はこの原文の言い換えである）

- > The first round of judging has no impact on your project's eligibility for partner prizes

  ライブ審査の一次選考は**パートナー賞の資格に影響しない**。総合で落ちてもパートナー賞は独立して取れる。

- > the majority of prizes are paid out to projects that do not advance

  非同期大会では**賞金の大半がライブ審査に進まない提出へ払われる**。
  つまり「枝葉を確実に拾う」は妥協ではなく、公式が記述している分布そのものである。

この2文が 0.5.1 の順位の一次根拠。**文言は各大会の details で毎回読み直す**（イベントごとに違い得る）。

### 0.5.3 パートナー賞コメントの型（次の大会でもこの3項で書く）

パートナーは自社サービスを**継続的な製品に継続して使ってほしい**からスポンサーしている。
だから審査員に見せるのは「使いました」ではなく「**外せなくなりました**」である。

| 項 | 書くこと | 通す関門 |
|---|---|---|
| **(a) 会期前にできなかったこと** | そのサービスを入れたことで、**会期前には構造的に不可能だった**何ができるようになったか。機能名ではなく、できなかった理由とできるようになった理由 | 会期前のコミットを指して「ここには無い」と言えるか |
| **(b) 会期後も必須部品として使い続ける具体** | **コードで裏が取れる約束だけ**書く。ファイルパスと行、または相手側に残っている実体（Gateway・レシピ・公開 URL）。「今後検討します」「拡張予定です」は書かない | 第三者がリポを clone して、その行を開けるか |
| **(c) x402 × そのサービスが開く用途** | こちらから提示する。パートナーは自社の道具が x402 とどう噛み合うかを我々ほど見ていない。**気づかせるのがこちらの仕事**であって、要件に答えるだけで終わらない | 提示した用途が、我々のコードで既に1つ動いているか |

**(b) の実測（2026-09-12・`git grep` を origin/main に対して実行）**

| パートナー | (b) に書ける根拠 | 状態 |
|---|---|---|
| The Graph | `packages/sdk/src/subgraph-evidence.ts`（製品パッケージ内。SDK の証拠源として `policy.evidence.source: "subgraph"` で選べる） | **【実測】製品コードにある。(b) をコードで書ける** |
| Bazantic | `apps/` と `packages/` に `bazantic` の一致は **0 件**。実体は `examples/ethonline-2026-ab/`（A/B ハーネス）と、**リポの外**にある Gateway・MCP・Recipe（`docs/ethonline-2026/BAZANTIC.md`） | **【実測】製品コードには無い。(b) は「相手側に何が残っているか」で書く**——Gateway URL・MCP の `initialize` 応答・bazantic.com 上の Recipe |

Bazantic のように**実体が外部にある**パートナーは、(b) を製品コードの行で担保できない。
そのときは「API が成功したか」ではなく「**相手側に何が残っているか**」を実測して書く（Gateway が LIVE であること、
`POST /mcp` の `initialize` が 200 を返すこと、Recipe が公開 URL で開くこと）。これを飛ばすと (b) は願望になる。

### 0.5.4 禁止 — 実在しない予定と、コードで裏が取れない約束

書いてはいけないもの。

- 実在しない予定（「次の大会で対応します」「ロードマップに入っています」）
- 実装を読まずに書いた仕様の要約
- 「必須の部品です」と言いながら、既定では通らない経路

**失敗例（2026-09-12・私が書きかけて撤回した実物。抽象化せずそのまま残す）**

> 「vet402 の SDK は既定の証拠源が subgraph（The Graph）である」

**これは誤り。** `packages/sdk/src/pay-or-refuse.ts` の実装で既定は `"vet402"` である（origin/main・2026-09-12 実測）。

```
600:  const l1Source = input.policy?.evidence?.source ?? "vet402";
657:  const wantedSource = input.policy?.evidence?.source ?? "vet402";
```

subgraph は**呼び出し側が `policy.evidence.source` で明示的に選んだときだけ**通る。
「必須の部品」を強く言いたい気持ちが、既定値の確認を1回飛ばさせた。
**パートナー賞コメントに書く一文ごとに、その行を開いてから書く。**

### 0.5.5 GTM の回収先 — 受賞そのものより資産が残る

賞金は副産物である。ハッカソンに出す本当の理由は、**提出物が資産として残り、後から読まれる**ことにある。

| 回収先 | 何が残るか |
|---|---|
| ETHGlobal Showcase | 提出ページ・デモ動画・リポへのリンクが会期後も公開され続ける |
| パートナーの発信 | 受賞・採用事例として先方のブログ／X／ドキュメントに名前が載る。こちらの発信より届く |
| Discord の露出 | 会期中にスポンサーのチャンネルで実物を見せた記録が、審査後も担当者に残る |
| 検索 | 提出物・README・公開レシピが、x402 を調べる人に後から読まれる |

だから**落選しても回収は成立する**。逆に、賞を取っても提出物が後から読めない形（動画だけ・リポ非公開・
壊れたリンク）なら GTM としては失敗である。提出前に「これは半年後に検索から読まれる形か」を1回問う。
配信面の運用は [`DISTRIBUTION.md`](./DISTRIBUTION.md)。
---
...
**2026-09-12 追記:** 上の2行（Finalist に残らなくても賞は取れる／賞金の大半はライブ審査に進まない提出へ）は、
同日 curl した details の原文で照合済み。原文の引用と、そこから導く資源配分の順位は **§0.5.2**。
...
**2026-09-12 追記（範囲の訂正）:** この節は ETHOnline 限定で書かれていたが、**同じ順位が全大会に効く**（§0.5.1）。
また「総合優勝は狙わない」の理由をここは書いていない——理由は賞金配分ではなく **GTM が第1目的だから**である（§0.5.1）。
パートナー賞コメントを書くときは **§0.5.3 の3項**を通す。
...
- **2026-09-12 追記:** Tokyo の賞コメントも **§0.5.3 の3項**（会期前にできなかったこと／コードで裏が取れる継続の約束／x402 との掛け合わせをこちらから提示）で書く。目的の順位は §0.5.1 で ETHOnline と同じ。
...
- **担当（2026-08-31 Takeshi 明示）: Devcon 8 と ETHGlobal Mumbai は渡航も含めて完全にハッカソン目的であり、ハッカソン戦略セッションが持つ。グラント案件ではない。** チケットの等級（GA / Public Goods / POAP）もこの渡航費の内訳であって、グラント側へ渡す論点ではない。
...
- **渡航の Go/No-Go ゲート（2026-08-28 決定・チケットと航空券は同時に判断する）**
  - **判定日: 2026-09-20**。ETHOnline 提出（9/13）の後、Tokyo（9/25）の前。渡航6週前で航空券もまだ動く。
  - **既定は No-Go。** 行くには **(A) エントリーが在ること** と **(B) 勝ち筋が在ること** の**両方**が要る。
    - (A) Mumbai の賞ページに、我々が資格を満たす **continuity 枠が1つ以上**ある
      （日次監視が公開を拾う。Tokyo は 5社 $45,000 で continuity 枠 **0** という前例があるので、有ると仮定しない）。
    - (B) その枠の要件が、会期中の動詞（**ERC-8004 Validation Registry への本書き込み**）と実際に噛み合う。
      「内容が近い」ではなく、要件を読んで**満たせると言い切れる**こと。
  - **面談は渡航の理由にしない。** 3件の会話のために $2,000 超と1週間は割に合わない。
    ERC-8004 / EF・Base・CDP・ENS・Arc への接触は Discord とオンラインで取りに行く（すでにその経路で動いている）。
  - **No-Go でも Registry 本書き込みは実装する。** 賞のためではなく堀のために。大会に出さないだけ。
  - 秋の本戦は **ETHOnline（無料・非同期）と Tokyo（現地だが日本国内で安い）**。Mumbai は上振れ枠であって前提ではない。
  - **判定日まで金を動かさない**。チケット（$499 ETH）も航空券も買わない。
  - ホテルは **(A) が出てからでよい**。押さえるなら**無料キャンセルのものだけ**（金は動かさない）。
    **Diwali 2026 は 11/8（祭は 11/6–10）**でハッカソン会期と完全に重なり、そこへ Devcon の1万人規模が乗るため、
    希少なのは航空券よりホテル——ただし希少性は「行くと決めた後」の話であって、行く理由にはならない。
