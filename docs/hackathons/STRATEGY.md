...
- **渡航の Go/No-Go ゲート（2026-08-28 決定・チケットと航空券は同時に判断する）**
  ...
  - **Go の条件（どちらか）**: (A) Mumbai の賞ページに、我々が資格を満たす **continuity 枠が1つ以上**ある
    （日次監視が公開を拾う。Tokyo は 5社 $45,000 で continuity 枠 **0** という前例があるので、有ると仮定しない）。
    (B) Devcon 期間の**面談が3件以上、相手と日時つきで書面確定**している。
  - **どちらも無ければ No-Go**。行かない。Mumbai の動詞（Registry 本書き込み）は、大会に出さなくても
    製品として実装してよい——賞のためではなく堀のために。
  - **判定日まで金を動かさない**。チケット（$499 ETH）も航空券も買わない。
  - ただし **ホテルだけは無料キャンセルで押さえる**。**Diwali 2026 は 11/8（祭は 11/6–10）**で、
    ハッカソン会期と完全に重なる。そこへ Devcon の1万人規模が乗るので、**希少なのは航空券よりホテル**。
    無料キャンセルなら金は動かず、No-Go でも損はしない。
