// ============================================================
// 公開 export の末尾 3 列と、遅延回収の件数の開示（2026-09-20）— 実 DB で固定する。
//
//  1. export.csv: 既存 11 列は 1 文字も変えず、末尾に request_body / request_body_sha256 /
//     settlement_source。記録の無い行は空で、empty・none・seller_claim に倒さない。
//  2. SQL（requestBodyKindSql・settlementSourceSql）と JS（requestBodyKindOf・settlementSourceOf）が
//     同じ台帳の同じ行に同じ値を出す。
...
if (!TEST_DB) {
  ...
  test("export.csv: 要求の形と tx の出所", async (t) => {
    ...
    type Seed = { label: string; status: string; http: number | null; txHash: string | null; meta: Record<string, unknown> | null; want: [string, string, string] };
    const seeds: Seed[] = [
    ...
    await t.test("export.csv: 既存 11 列はそのまま・末尾 3 列・記録の無い行は空", async () => {
      ...
      assert.equal(
        ...
        "attempted_at,resource_key,network,status,amount_units,spent_units,tx_hash,http_status_paid,latency_ms,l2_schema,held_reason," +
          "request_body,request_body_sha256,settlement_source",
      );
      assert.equal(lines.length - 1, seeds.length, "budget_denied は出ない");
      for (const [i, s] of seeds.entries()) {
        const cells = lines[i + 1].split(",");
        assert.equal(cells.length, 14, s.label);
    ...
    await t.test("SQL と JS が同じ行に同じ値を出す", async () => {
      ...
      const rows = rowsOf<{ tx_hash: string | null; raw_response_meta: unknown; request_body: string | null; request_body_sha256: string | null; settlement_source: string | null }>(raw);
