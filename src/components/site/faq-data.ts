...
export const FAQS: FaqItem[] = [
  ...
  },
  {
    ...
    answer:
      "settled means vet402 re-read the transaction on-chain and found the exact USDC transfer it paid for: from our payer, to the catalog-declared payee, for the declared amount, in the canonical USDC contract. It is never inferred from the seller's own claim: a receipt the seller returns is recorded as settle_claimed until we have re-read it. delivered is the separate count — the attempt is settled and the paid request also answered 2xx. A settled attempt that answered 4xx is published as inconclusive instead of counted against the seller: vet402 buys with an empty JSON body and no API key of the seller's, so a 4xx cannot be separated from a request vet402 formed wrongly. Those rows stay published with their HTTP code and are out of the denominator for delivered.",
  },
  {
