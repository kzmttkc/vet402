    ...
    return NextResponse.json(
      {
        ...result,
        disclaimer:
          "Facts only: attempts, settlements and their on-chain receipts. settledCount is the transfer vet402 re-read on-chain; deliveredCount is the subset whose paid request also answered 2xx; inconclusiveCount is the paid attempts held rather than counted against the seller, split in inconclusiveByReason: settled4xx (settled, and the paid request answered 4xx), unsettled4xx (a 4xx other than 402 with no settlement receipt — a seller that validates the request before settling it lands here) and payerUnfunded (a 402 or 5xx while vet402's own Base payer wallet was out of USDC, 2026-09-13T00:00Z–2026-09-15T23:49Z). vet402 buys with no API key of the seller's and sends a POST body only when the seller declares one, so it cannot rule out that a 4xx was its own request to get wrong. inconclusiveSettledCount is the settled part; deliveryRatePct is deliveredCount over settledCount minus inconclusiveSettledCount — the settled attempts vet402 can actually judge. This is a record of what happened when vet402 paid this endpoint, not an endorsement or a prediction.",
