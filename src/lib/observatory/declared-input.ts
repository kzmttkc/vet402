 ...
 *   2. 並び順は `Object.entries` の順＝宣言のキー順。それをソートし直したりはしない。
 *      ただし**整数に見える名前だけは JS の規則で先頭に昇順で来る**（宣言が `10, 2, x, 1` なら `1=…&2=…&10=…&x=…`）。
 *      「JSON に書いてある順」で再計算すると別のハッシュになる（2026-09-20 再レビュー N-10）。
 *   3. 符号化は `application/x-www-form-urlencoded`（URLSearchParams）——空白は `+`。
 *      `encodeURIComponent` では一致しない（`~` `!` `*` と空白の扱いが違う）。
