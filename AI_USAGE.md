...
```bash
git rev-list --count HEAD                                  # 754 commits (2026-09-07)
git log --grep='Co-Authored-By: Claude' --oneline | wc -l  # 635 carry the AI trailer
git rev-list --count --merges HEAD                         # 34 are merge commits
```

**The <!-- n:no_trailer_commits -->119<!-- /n --> without the trailer are not "written by a human".** <!-- n:merge_commits -->34<!-- /n --> are merges; most of the rest are AI
...
```bash
git diff --diff-filter=A --name-only pre-ethonline-2026..main | wc -l   # 178 added   (2026-09-07)
git diff --diff-filter=M --name-only pre-ethonline-2026..main | wc -l   # 174 modified
```
