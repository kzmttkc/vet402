...
```bash
git rev-list --count HEAD                                  # 799 commits (2026-09-08)
git log --grep='Co-Authored-By: Claude' --oneline | wc -l  # 669 carry the AI trailer
...
```bash
git diff --diff-filter=A --name-only pre-ethonline-2026..main | wc -l   # 199 added   (2026-09-08)
git diff --diff-filter=M --name-only pre-ethonline-2026..main | wc -l   # 187 modified
```
