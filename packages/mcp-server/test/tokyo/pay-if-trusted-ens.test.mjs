// ETHGlobal Tokyo 2026 — B1 の red テスト: MCP の pay_if_trusted に ENSIP-29 段を通す（切り捨て #1 の対象）。
// 正典: PLAN_v4.3 §3.9・§5（T38）。モジュールはテストの中で await import する。
// ENS は EnsRpc の偽物2つ、HTTP は偽 fetch。ネットワークに出ない。
...
const API = "https://api.vet402.test"; // 偽の apiUrl（定数）

test("T38 pay_if_trusted に payeeName（偽の /decision は ALLOW・ENS は T02 の1文字違い）→ REFUSE ens_attestation_signer_mismatch・/decision への fetch 0 本", { skip: !process.env.TOKYO_OPT_MCP }, async () => {
  const { payIfTrusted } = await import("../../dist/pay-if-trusted.js");
  const w = ensWorld();
  w.texts["seller-a.eth"][KEY_OFFER] = OFFER_1CHAR;
  const { clients } = ensClients(w);
  const accessed = [];
  const signer = new Proxy({ address: "0xDB62BD202914609830fA656F87996b91be3Aa673", signTypedData: async () => "0x" + "11".repeat(65) },
    { get: (t, p) => (accessed.push(String(p)), Reflect.get(t, p)) });
  const calls = [];
  const fetch = async (u) => {
    calls.push(String(u));
    if (String(u).includes("/decision")) {
      return { ok: true, status: 200, headers: new Headers(), json: async () => ({ recommendation: "ALLOW", reason_codes: ["l0_pass"], facts: {}, evidence: [], degraded: false }) };
    }
    throw new Error(`forbidden call: ${u}`);
  };
  const r = await payIfTrusted({
    resourceId: "b".repeat(64), signer, fetch, apiUrl: API,
    payeeName: "seller-a.eth", network: "base-sepolia", resource: RESOURCE, method: "GET", amountUsd: 0.01,
    ens: { clients, policy: ensPolicy(w) },
  });
  assert.equal(r.decision, "REFUSE");
  assert.equal(r.refuse_reasons.includes("ens_attestation_signer_mismatch"), true, JSON.stringify(r.refuse_reasons));
  assert.equal(calls.filter((u) => u.includes("/decision")).length, 0, "/decision への fetch 0 本");
  assert.equal(calls.length, 0, "HTTP に1本も出ない");
  assert.equal(r.safe_to_pay, false);
  assert.equal(r.nonce, null);
  assert.deepEqual(accessed.filter((k) => k.startsWith("sign")), []);
