...
| `VOUCH_SOLANA_PAYER_SECRET_KEY` | — | Solana payer for `pay_if_trusted` when `payee` is a base58 address (2026-09-15). `solana-keygen` JSON array or base64 of the 64-byte secret key. Needs `SOLANA_RPC_URL` and `@solana/web3.js` installed next to the server; without them a base58 payee refuses with `payer_not_configured`. The Base payer (`VOUCH_PAYER_PRIVATE_KEY`) is never used for a Solana payee, or the reverse. Never commit or print a real value. |
| `SOLANA_RPC_URL` | — | Solana RPC the payer reads one `getLatestBlockhash` from, only after an `ALLOW` (2026-09-15). |
