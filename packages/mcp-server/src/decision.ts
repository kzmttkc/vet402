...
function isExpired(cacheExpiresAt: unknown, now: number): boolean {
  if (typeof cacheExpiresAt !== "string" || cacheExpiresAt.length === 0) return false;
 ...
 */
export function decideFromScore(score: unknown, now: number = Date.now()): TrustDecision {
  if (score === null || typeof score !== "object") {
    return refuse(["malformed_response"], "The trust API did not return a readable result.");
  }
  const s = score as ScoreLike;

  const reasons: RefuseReason[] = [];

  if (s.degraded === true) reasons.push("degraded_measurement");

  const unavailable = Array.isArray(s.signalsUnavailable) ? s.signalsUnavailable : [];
  if (unavailable.length > 0) reasons.push("partial_measurement");

  if (isExpired(s.cacheExpiresAt, now)) reasons.push("score_stale");

  if (s.recommendation !== "ALLOW") reasons.push("recommendation_not_allow");
...
function refuse(reasons: RefuseReason[], summary: string): TrustDecision {
  return { decision: "REFUSE", safe_to_pay: false, refuse_reasons: reasons, summary };
}

function refuseSummary(reasons: RefuseReason[], unavailable: unknown[]): string {
  const parts: string[] = [];
  ...
  if (reasons.includes("score_stale")) parts.push("the cached score has expired");
  if (reasons.includes("recommendation_not_allow")) parts.push("the recommendation is not ALLOW");
