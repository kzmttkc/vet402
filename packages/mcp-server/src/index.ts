...
const UNCONFIGURED_SIGNER: PayIfTrustedSigner = {
 ...
 */
async function resolvePayer(): Promise<PayIfTrustedSigner | null> {
...
server.tool(
  ...
  [
    ...
    "with each row's own source (vet402 or subgraph) so you can check which ledger answered. A",
    ...
    "with a payment target). Paying also requires VOUCH_PAYER_PRIVATE_KEY in this server's env and viem",
  ...
  async ({ resourceId, resource, payee, amountUsd, method, maxPerTxUsd, policy }) => {
    try {
      ...
      if (notes.length > 0) result.summary = [result.summary, ...notes].filter(Boolean).join(" ");
