 ...
 */
import { SOLANA_PAYEE_RE, type PayIfTrustedSvmSigner } from "./pay-if-trusted.js";

/** 触れられたら throw する Solana の署名者。未設定のとき payIfTrusted へ渡す値で、到達しない（EVM の UNCONFIGURED_SIGNER と同じ役）。 */
export const UNCONFIGURED_SVM_SIGNER: PayIfTrustedSvmSigner = {
 ...
 */
export function payerConfiguredFor(payee: unknown, evmSigner: unknown, svmSigner: unknown): boolean {
  if (typeof payee === "string" && SOLANA_PAYEE_RE.test(payee)) return svmSigner !== null;
 ...
 */
export async function resolveSvmPayer(
  secretKey: string | undefined,
  rpcUrl: string | undefined,
): Promise<{ signer: PayIfTrustedSvmSigner; rpcUrl: string } | null> {
  const raw = secretKey?.trim() ?? "";
  const rpc = rpcUrl?.trim() ?? "";
  if (raw === "" || !/^https?:\/\//.test(rpc)) return null;
  try {
    // 変数指定子。web3.js を静的依存にしない（未インストールでもビルドと起動が通る・viem と同じ手）。
    const specifier = "@solana/web3.js";
    const web3 = (await import(specifier)) as {
      Keypair: { fromSecretKey(secret: Uint8Array): { publicKey: { toBase58(): string }; secretKey: Uint8Array } };
    };
    const secret = raw.startsWith("[")
      ? Uint8Array.from(JSON.parse(raw) as number[])
      : Uint8Array.from(atob(raw), (c) => c.charCodeAt(0));
    const keypair = web3.Keypair.fromSecretKey(secret);
    const signer: PayIfTrustedSvmSigner = {
      address: keypair.publicKey.toBase58(),
      // SDK の `svmAccountFromKeypair` と同じ形。SDK をここで値として import すると、拒否経路でも SDK が
      // 評価される（pay-if-trusted.ts の第3層が崩れる）ので写す。
      signTransaction: async (tx) => {
        const signable = tx as typeof tx & { sign?: (signers: unknown[]) => void };
        if (typeof signable.sign !== "function") throw new Error("invalid_svm_transaction: expected a VersionedTransaction");
