 ...
 */
export function resolveMaxPerTxUsd(requested, rawEnv) {
    ...
    const wanted = typeof requested === "number" && Number.isFinite(requested) && requested > 0 ? requested : undefined;
    return {
 ...
 */
export function ceilingNotes(r) {
    const notes = [];
    if (r.envRejected) {
        notes.push(`${MAX_PER_TX_USD_ENV} in this server's env is not a positive number, so it was ignored and the ` +
    ...
    if (r.clamped) {
        notes.push(`Server ceiling: this MCP server caps a single payment at $${r.ceiling}` +
