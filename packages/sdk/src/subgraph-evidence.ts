...
export type SubgraphUnavailable = { ok: false; error: string };
 ...
 */
export function redactGraphKey(text: string, apiKey?: string): string {
  let out = String(text);
  if (typeof apiKey === "string") {
    const key = apiKey.trim();
    if (key.length >= MIN_GRAPH_KEY_LENGTH) out = out.replace(new RegExp(escapeRegExp(key), "gi"), GRAPH_KEY_PLACEHOLDER);
  }
...
async function readSubgraphReceiptsUnredacted(input: ReadSubgraphReceiptsInput): Promise<SubgraphReadResult> {
  ...
  try {
    ...
    if (response.ok !== true) return { ok: false, error: `graph_http_${response.status}` };
    body = await response.json();
  } catch (error) {
    return { ok: false, error: `graph_unreachable: ${error instanceof Error ? error.message : String(error)}` };
  }

  if (typeof body !== "object" || body === null) return { ok: false, error: "graph_malformed_response" };
  ...
  const data = envelope.data as
    | { x402AddressSummaries?: unknown; _meta?: { block?: { number?: unknown; timestamp?: unknown }; deployment?: unknown } }
    | undefined;
  ...
  return {
    ...
    block: {
      number: blockNumber,
      ...(typeof data?._meta?.block?.timestamp === "number" ? { timestamp: data._meta.block.timestamp as number } : {}),
    },
