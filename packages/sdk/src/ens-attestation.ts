...
import {
  attestationKey, bytesToHex, decodeEnvelope, encodePayload, profileOfVersion,
  type AtstProfile, type DecodedEnvelope,
} from "./atst-codec.js";
import {
  ENS_REASON_PRIORITY, type EnsAttestationReason,
} from "./ens-reasons.js";
import {
  EnsEvidenceUnavailable, ENS_SEPOLIA_CHAIN_ID, findExactOwner, getState, normalizeName, pinBlock,
  resolveAddr, resolveText, type EnsReadClients,
} from "./ens-read.js";
...
export type X402Offer = {
  ...
  network: "eip155:84532"; asset: `0x${string}`; amount: string /* decimal uint */; payTo: `0x${string}`;
  output?: { required: string[] };
};

export type TrustedAttester = {
  name: string;                         // e.g. "atst.vet402.eth"
  address: `0x${string}`;               // pinned. If the name resolves elsewhere → ens_attester_unpinned
  recordKeys: readonly string[];        // record types this attester is trusted for (draft line 44)
  anchor?: { name: string; owner: `0x${string}` }; // pinned owner of the parent name
};

export type EnsAttestationPolicy = {
  recordKey?: "x402-offer";
  trustedAttesters: readonly TrustedAttester[];
  minValid?: number;            // default 1. 1..(attesters trusted for recordKey)
  maxAgeSeconds?: number;       // default 86_400. 60..604_800
  futureSkewSeconds?: number;   // default 300. 0..900
  maxHeadLagSeconds?: number;   // default 120. 30..600
  profiles?: readonly AtstProfile[];               // default ["ensip29-draft"]
  sinceIssuanceScan?: false | { canary?: { address: `0x${string}`; blockNumber: bigint; txHash: `0x${string}` } };
  now?: () => number;           // clock in seconds (tests)
};

/** The two fields of a chain profile this gate reads (the base-sepolia row of chain-profile.ts). */
export type EnsOfferProfile = { network: string; asset: string; [k: string]: unknown };

export type AttestationStep = { step: 1 | 2 | 3 | 4 | 5 | 6 | 7; status: "ok" | "fail" | "skipped"; detail: Record<string, string> };

export type AttestationResult = {
  attester: string; profile: AtstProfile | null; t: number | null;
...
export type EnsOfferCheck = {
  ...
  trace: AttestationStep[];     // always 7 entries
};
...
const DEFAULTS = { recordKey: "x402-offer", minValid: 1, maxAgeSeconds: 86_400, futureSkewSeconds: 300, maxHeadLagSeconds: 120 } as const;
const ENDPOINT_KEY = "agent-endpoint[x402]";
const ALL_PROFILES: readonly AtstProfile[] = ["ensip29-draft", "atst-me-v2"];
...
const SCAN_SPAN = 1_000n;

/** The scan needs a fifth method on both RPCs. viem's PublicClient has it. */
type EnsLogRpc = { getLogs(args: any): Promise<unknown> };
const hasGetLogs = (c: unknown): c is EnsLogRpc => typeof (c as { getLogs?: unknown } | null)?.getLogs === "function";
 ...
 */
async function scanSinceIssuance(
  clients: EnsReadClients, B: bigint, ts: bigint, oldestT: number, resolver: `0x${string}`, node: `0x${string}`,
  recordKey: string, canary: { address: `0x${string}`; blockNumber: bigint; txHash: `0x${string}` } | undefined,
): Promise<{ changes: number; fromBlock: bigint }> {
  const t = BigInt(oldestT);
  const back = t >= ts ? 0n : (ts - t + SLOT_SECONDS - 1n) / SLOT_SECONDS + 1n;
  const fromBlock = B > back ? B - back : 0n;
  const sides = [clients.primary, clients.secondary] as unknown as EnsLogRpc[];
  const logsOf = async (c: EnsLogRpc, args: Record<string, unknown>): Promise<unknown[]> => {
    const out = await c.getLogs({ ...args, strict: false });
    if (!Array.isArray(out)) throw new EnsEvidenceUnavailable("getLogs did not return an array");
    return out;
  };
  let changes = 0;
  try {
    ...
    if (changes === 0 && canary) {
      const seen = await Promise.all(sides.map((c) =>
        logsOf(c, { address: canary.address, fromBlock: canary.blockNumber, toBlock: canary.blockNumber })));
      const hasCanary = (logs: unknown[]) => logs.some((l) => lc((l as { transactionHash?: string } | null)?.transactionHash) === lc(canary.txHash));
      if (!seen.every(hasCanary)) throw new EnsEvidenceUnavailable(`the canary log ${canary.txHash} was not returned by both RPCs, so an empty scan proves nothing`);
    }
  } catch (e) {
    if (e instanceof EnsEvidenceUnavailable) throw e;
    throw new EnsEvidenceUnavailable(`since-issuance scan failed: ${String((e as Error)?.message ?? e).slice(0, 200)}`);
  }
...
function intIn(v: unknown, lo: number, hi: number, what: string): void {
  if (!Number.isInteger(v) || (v as number) < lo || (v as number) > hi) bad(`${what} must be an integer in ${lo}..${hi} (got ${String(v)})`);
}

/** Throws `invalid_attestation_policy: …` before any network access if one field is off. */
export function assertEnsAttestationPolicy(p: unknown): asserts p is EnsAttestationPolicy {
  if (!p || typeof p !== "object" || Array.isArray(p)) bad("policy must be an object");
  const o = p as Record<string, unknown>;
  for (const k of Object.keys(o)) if (!POLICY_KEYS.has(k)) bad(`unknown key ${JSON.stringify(k)}`);
  if (o.recordKey !== undefined && o.recordKey !== "x402-offer") bad(`recordKey must be "x402-offer"`);
  const recordKey = (o.recordKey as string | undefined) ?? DEFAULTS.recordKey;
  const ta = o.trustedAttesters;
  if (!Array.isArray(ta) || ta.length === 0) bad("trustedAttesters must be a non-empty array");
  for (const [i, a] of (ta as unknown[]).entries()) {
    const x = a as Record<string, unknown>;
    if (!x || typeof x !== "object") bad(`trustedAttesters[${i}] must be an object`);
    if (typeof x.name !== "string" || x.name.length === 0 || !x.name.includes(".")) bad(`trustedAttesters[${i}].name must be an ENS name`);
    if (typeof x.address !== "string" || !ADDRESS_RE.test(x.address)) bad(`trustedAttesters[${i}].address must be a 20-byte hex address`);
    ...
    if (x.anchor !== undefined) {
      const an = x.anchor as Record<string, unknown>;
  ...
  const trustedForKey = (ta as TrustedAttester[]).filter((a) => a.recordKeys.includes(recordKey)).length;
  if (o.minValid !== undefined) intIn(o.minValid, 1, Math.max(1, trustedForKey), "minValid");
  if (trustedForKey < ((o.minValid as number | undefined) ?? DEFAULTS.minValid)) bad(`no trusted attester for ${recordKey} can satisfy minValid`);
  if (o.maxAgeSeconds !== undefined) intIn(o.maxAgeSeconds, 60, 604_800, "maxAgeSeconds");
  if (o.futureSkewSeconds !== undefined) intIn(o.futureSkewSeconds, 0, 900, "futureSkewSeconds");
  if (o.maxHeadLagSeconds !== undefined) intIn(o.maxHeadLagSeconds, 30, 600, "maxHeadLagSeconds");
  if (o.profiles !== undefined) {
    const pr = o.profiles;
    if (!Array.isArray(pr) || pr.length === 0) bad("profiles must be a non-empty array");
    if (!pr.every((x) => ALL_PROFILES.includes(x as AtstProfile))) bad(`profiles must be "ensip29-draft" or "atst-me-v2"`);
    if (new Set(pr).size !== pr.length) bad("profiles must not repeat");
  }
  if (o.sinceIssuanceScan !== undefined && o.sinceIssuanceScan !== false) {
    const s = o.sinceIssuanceScan as Record<string, unknown>;
    if (!s || typeof s !== "object") bad("sinceIssuanceScan must be false or {canary?}");
    if (s.canary !== undefined) {
      const c = s.canary as Record<string, unknown>;
  ...
  if (o.now !== undefined && typeof o.now !== "function") bad("now must be a function");
}

// ---------- offer ----------

function parseOffer(raw: string): X402Offer | null {
  let o: any;
  try {
    o = JSON.parse(raw);
  } catch {
  ...
  if (typeof o.network !== "string" || typeof o.asset !== "string" || !ADDRESS_RE.test(o.asset)) return null;
  if (typeof o.amount !== "string" || !/^(0|[1-9][0-9]*)$/.test(o.amount)) return null;
  if (typeof o.payTo !== "string" || !ADDRESS_RE.test(o.payTo)) return null;
  if (o.output !== undefined) {
    if (!o.output || typeof o.output !== "object" || !Array.isArray(o.output.required) || !o.output.required.every((k: unknown) => typeof k === "string")) return null;
  }
  return o as X402Offer;
}

const NETWORK_SLUGS: Record<string, string> = { "base-sepolia": "eip155:84532", base: "eip155:8453" };
 ...
 */
export function compareOfferToAccept(
  o: X402Offer,
  a: { payTo?: string; network?: string; asset?: string; maxAmountRequired?: string | number | bigint; amount?: string | number | bigint },
): ("payee_mismatch" | "price_above_declared" | "chain_or_asset_mismatch")[] {
  const out: ("payee_mismatch" | "price_above_declared" | "chain_or_asset_mismatch")[] = [];
  if (!a || lc(a.payTo) !== lc(o.payTo)) out.push("payee_mismatch");
  ...
  if (price === null || price > BigInt(o.amount)) out.push("price_above_declared");
  const net = NETWORK_SLUGS[String(a?.network)] ?? String(a?.network);
  if (net !== o.network || lc(a?.asset) !== lc(o.asset)) out.push("chain_or_asset_mismatch");
  return out;
}

// ---------- the gate ----------

type Fail = { step: 1 | 2 | 3 | 4 | 5 | 6 | 7; detail: Record<string, string> };

type AttesterWork = {
  cfg: TrustedAttester;
  result: AttestationResult;
  envRaw: string;
  env: DecodedEnvelope | null;
  resolved: `0x${string}` | null;
  anchorOwner: `0x${string}` | null | undefined;
  failStep: number; // 8 = passed all seven
  details: Partial<Record<1 | 2 | 3 | 4 | 5 | 6 | 7, Record<string, string>>>;
};

function buildTrace(fail: Fail | null, details: Partial<Record<number, Record<string, string>>>): AttestationStep[] {
  const steps: AttestationStep[] = [];
  for (let s = 1 as AttestationStep["step"]; s <= 7; s = (s + 1) as AttestationStep["step"]) {
    if (fail && s === fail.step) steps.push({ step: s, status: "fail", detail: { ...(details[s] ?? {}), ...fail.detail } });
    else if (fail && s > fail.step) steps.push({ step: s, status: "skipped", detail: {} });
    else steps.push({ step: s, status: "ok", detail: details[s] ?? {} });
  }
  return steps;
}

function sortReasons(r: Iterable<EnsAttestationReason>): EnsAttestationReason[] {
 ...
 */
export async function checkEnsOffer(input: CheckEnsOfferInput): Promise<EnsOfferCheck> {
  ...
  const profiles = policy.profiles ?? (["ensip29-draft"] as const);
  ...
  const out: EnsOfferCheck = {
    ok: false, reason_codes: [], name: input.name, node: "0x" as `0x${string}`, chainId: ENS_SEPOLIA_CHAIN_ID,
  ...
  const details: Partial<Record<number, Record<string, string>>> = {};
  const finish = (reasons: EnsAttestationReason[], fail: Fail | null): EnsOfferCheck => {
  ...
  try {
    ...
    return finish(["ens_name_unresolved"], { step: 1, detail: { reason: "ens_name_unresolved", error: `not a valid ENS name: ${(e as Error).message}`.slice(0, 200) } });
  }
  out.name = name;
  out.node = namehash(name);

  const works: AttesterWork[] = trusted.map((cfg) => ({
    cfg,
  ...
  let stage: Fail["step"] = 1;
  let offerResolver: `0x${string}` | null = null;
  try {
    ...
    stage = 1;
    for (const w of works) w.envRaw = (await resolveText(clients, B, name, attestationKey(recordKey, w.cfg.name))).value;
    ...
    const unresolved = (error: string) =>
      finish(["ens_name_unresolved"], { step: anyEnvelope ? 2 : 1, detail: { reason: "ens_name_unresolved", error } });
    if (!owner) return unresolved("findExactOwner returned 0x0 (unregistered, expired, reserved or an unregistered subname)");
    ...
    for (const w of works) {
      if (!w.envRaw || out.offerRaw === "") continue;
      const d = decodeEnvelope(w.envRaw);
      if ("error" in d || !profiles.includes(profileOfVersion(d.version))) continue;
      w.resolved = (await resolveAddr(clients, B, w.cfg.name)).value;
      if (w.cfg.anchor) w.anchorOwner = await findExactOwner(clients, B, normalizeName(w.cfg.anchor.name));
    }
  } catch (e) {
  ...
  for (const w of works) {
    const r = w.result;
    const fail = (step: number, reason: EnsAttestationReason | null, d: Record<string, string>) => {
      w.failStep = step;
      r.reason = reason;
      w.details[step as 1] = { ...(w.details[step as 1] ?? {}), ...(reason ? { reason } : {}), ...d };
    };
    const key = attestationKey(recordKey, w.cfg.name);
    w.details[1] = { attester: w.cfg.name, key };
    if (!w.envRaw) { fail(1, "ens_attestation_missing", { error: "no envelope under this key" }); continue; }
    const d = decodeEnvelope(w.envRaw);
    if ("error" in d) { fail(1, "ens_attestation_malformed", { error: d.error }); continue; }
    const prof = profileOfVersion(d.version);
    if (!profiles.includes(prof)) { fail(1, "ens_attestation_malformed", { error: `envelope version ${d.version} (${prof}) is not in profiles [${profiles.join(",")}]` }); continue; }
    w.env = d;
    r.profile = prof;
    r.t = d.t;
    w.details[1] = { ...w.details[1], encoding: w.envRaw.startsWith("0x") ? "hex" : "base64", version: String(d.version), t: String(d.t) };
    if (out.offerRaw === "") { w.failStep = 3; continue; }

    let payload: Uint8Array;
    try {
      payload = encodePayload({ n: name, a: out.manager as `0x${string}`, k: recordKey, v: out.offerRaw as string, t: d.t }, prof);
    } catch (e) { fail(4, "ens_attestation_malformed", { error: (e as Error).message.slice(0, 200) }); continue; }
    ...
    } catch (e) { fail(5, "ens_attestation_malformed", { digest: r.digest, error: (e as Error).message.slice(0, 200) }); continue; }
    ...
    if (w.resolved === null) { fail(6, "ens_attester_unresolved", { error: `${w.cfg.name} has no addr` }); continue; }
    if (lc(w.resolved) !== lc(w.cfg.address)) { fail(6, "ens_attester_unpinned", { error: `${w.cfg.name} resolves to ${w.resolved}, pinned ${w.cfg.address}` }); continue; }
    if (w.cfg.anchor && lc(w.anchorOwner) !== lc(w.cfg.anchor.owner)) {
      fail(6, "ens_attester_anchor_changed", { error: `owner of ${w.cfg.anchor.name} is ${String(w.anchorOwner)}, pinned ${w.cfg.anchor.owner}` }); continue;
    }

    if (lc(r.recovered) !== lc(w.resolved)) { fail(7, "ens_attestation_signer_mismatch", { recovered: r.recovered, expected: w.resolved }); continue; }
    if (now - d.t > maxAge) { fail(7, "ens_attestation_stale", { error: `issued ${now - d.t}s ago (max ${maxAge}s)` }); continue; }
    if (d.t - now > skew) { fail(7, "ens_attestation_stale", { error: `issued ${d.t - now}s in the future (max skew ${skew}s)` }); continue; }
    r.valid = true;
    w.failStep = 8;
  ...
  if (scan && out.offerRaw !== "" && validWorks.length > 0) {
    try {
      if (!offerResolver) throw new EnsEvidenceUnavailable(`no resolver answered ${recordKey}, so there is nothing to scan`);
      const oldestT = Math.min(...validWorks.map((w) => w.result.t as number));
  ...
  const best = works.reduce<AttesterWork | null>((b, w) => (b === null || w.failStep > b.failStep ? w : b), null);
  for (const s of [1, 4, 5, 6, 7] as const) if (best?.details[s]) details[s] = best.details[s];
  ...
  if (validCount < minValid) {
    for (const w of works) if (!w.result.valid && w.result.reason) reasons.push(w.result.reason);
    if (reasons.length === 0) reasons.push("ens_attestation_missing");
    const step = Math.min(best?.failStep ?? 1, 7) as Fail["step"];
    fail = { step, detail: step === 7 && best?.failStep === 8 ? { reason: "ens_attestation_missing", error: `${validCount} valid < minValid ${minValid}` } : {} };
  }

  const offer = parseOffer(out.offerRaw as string);
  out.offer = offer;
  if (!offer) {
    ...
    const why: string[] = [];
    if (offer.resource !== resource) why.push(`resource ${offer.resource} ≠ ${resource}`);
    if (offer.method !== method) why.push(`method ${offer.method} ≠ ${method}`);
    if (out.endpoint !== offer.resource) why.push(`${ENDPOINT_KEY} ${JSON.stringify(out.endpoint)} ≠ offer.resource`);
    if (offer.network !== profile.network) why.push(`network ${offer.network} ≠ ${profile.network}`);
    if (lc(offer.asset) !== lc(profile.asset)) why.push(`asset ${offer.asset} ≠ ${profile.asset}`);
    if (why.length) {
      reasons.push("ens_offer_mismatch");
      fail ??= { step: 7, detail: { reason: "ens_offer_mismatch", error: why.join("; ").slice(0, 300) } };
    }
  }
  if (validCount >= minValid) details[7] = { ...(details[7] ?? {}), valid: `${validCount}/${minValid}` };
