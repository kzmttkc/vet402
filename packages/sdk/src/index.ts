...
export {
  ...
  PAY_REFUSE_REASONS,
  // 2026-09-15: Solana（SVM exact）。payee が base58 なら svm の署名者で払う。web3.js は ALLOW でだけ読む。
  SOLANA_MAINNET,
  SOLANA_USDC,
  svmAccountFromKeypair,
  type PayRail,
  type SvmPayOptions,
  type SvmPayerAccount,
  type SvmTransactionLike,
  type SvmKeypairLike,
 ...
 */
// ---- 製品定義書 §9.1（2026-09-02）: /decision と /resolve（読み取りのみ） ----
// 買い手側で「署名前に止める」配線（SpendGuard の切替）は 9/4 00:00 UTC 以降に入れる。
// ここは判定を読むメソッドだけを足す。
...
export type ResolveResult = {
  query: { kind: "url" | "domain" | "address" | "tx" | "payee_id" | "unknown"; value: string };
...
export class VouchClient {
  ...
  resolve(q: string): Promise<ResolveResult> {
    if (typeof q !== "string" || q.trim().length === 0) throw new Error("invalid_query");
