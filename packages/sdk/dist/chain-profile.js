...
export function profileFor(network) {
    if (network === undefined)
        return CHAIN_PROFILES.base;
    if (network === "base" || network === "base-sepolia")
        return CHAIN_PROFILES[network];
    throw new Error(`invalid_network: network must be "base" (default) or "base-sepolia", got ${JSON.stringify(network)}`);
}
/** CAIP-2 の network から profile を引く（知らなければ null）。v1 のスラッグを書くときに使う。 */
export function profileForCaip2(caip2) {
    if (caip2 === CHAIN_PROFILES.base.network)
        return CHAIN_PROFILES.base;
    if (caip2 === CHAIN_PROFILES["base-sepolia"].network)
        return CHAIN_PROFILES["base-sepolia"];
    return null;
}
