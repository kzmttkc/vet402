...
export function dnsEncode(name) {
    ...
    for (const label of name.split(".")) {
        ...
        if (b.length === 0 || b.length > 255)
            throw new Error(`dnsEncode: bad label length in ${JSON.stringify(name)}`);
        out.push(b.length, ...b);
    }
    out.push(0);
...
function revertIdentity(e) {
    let cur = e;
    for (let i = 0; cur && i < 8; i++) {
        if (cur.name === "ContractFunctionRevertedError")
            return String(cur.raw ?? cur.signature ?? cur.reason ?? "");
        cur = cur.cause;
    }
    return null;
}
async function one(c, fn, allowRevert, side) {
    try {
 ...
 */
export async function pinBlock(clients, maxHeadLagSeconds, nowSeconds = Math.floor(Date.now() / 1000)) {
    const call = async (side, f) => {
        try {
    ...
    if (diff > MAX_HEAD_DIFF)
        throw new EnsEvidenceUnavailable(`heads are ${diff} blocks apart (primary ${hp}, secondary ${hs}; max ${MAX_HEAD_DIFF})`);
    const B = hp < hs ? hp : hs;
    const [bp, bs] = await Promise.all([call("primary", (c) => c.getBlock({ blockNumber: B })), call("secondary", (c) => c.getBlock({ blockNumber: B }))]);
    if (bp.timestamp !== bs.timestamp)
        throw new EnsEvidenceUnavailable(`block ${B} has different timestamps on the two RPCs`);
    const ts = bp.timestamp;
    const lag = BigInt(Math.floor(nowSeconds)) - ts;
    if (lag > BigInt(maxHeadLagSeconds))
        throw new EnsEvidenceUnavailable(`head is ${lag}s old (max ${maxHeadLagSeconds}s)`);
...
const addrOrNull = (a) => {
    const s = String(a ?? "");
    if (!/^0x[0-9a-fA-F]{40}$/.test(s) || s.toLowerCase() === ZERO_ADDRESS)
        return null;
    return getAddress(s);
};
/** Exact owner of the (normalized) name, or null for 0x0 (unregistered, expired, RESERVED, wildcard subname). */
export async function findExactOwner(clients, B, name) {
...
async function urResolve(clients, B, name, data, what) {
    const dns = dnsEncode(name);
    ...
    if (!r.ok)
        return { result: "0x", resolver: null };
    const [result, resolver] = r.value;
    return { result: (result ?? "0x"), resolver: addrOrNull(resolver) };
}
/** text(name, key) through UniversalResolver.resolve, with the resolver that answered. "" when empty. */
export async function resolveText(clients, B, name, key) {
...
export async function resolveAddr(clients, B, name) {
...
export async function getState(clients, B, label) {
    const id = BigInt(labelhash(label));
    const s = (await readBoth(clients, (c) => c.readContract({ address: ENS_SEPOLIA.ethRegistry, abi: REGISTRY_ABI, functionName: "getState", args: [id], blockNumber: B }), `getState(${label})`));
    return {
        tokenId: BigInt(s.tokenId),
        status: Number(s.status),
        latestOwner: (addrOrNull(s.latestOwner) ?? ZERO_ADDRESS),
