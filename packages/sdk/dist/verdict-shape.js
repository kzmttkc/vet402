 ...
 */
export function isBlockVerdict(value) {
    return String(value ?? "").trim().toUpperCase() === "BLOCK";
}
/**
 ...
 */
export function scoreQualityDefect(score) {
    if (score === null || typeof score !== "object")
        return "degraded";
    if (typeof score.degraded !== "boolean" || score.degraded === true)
        return "degraded";
    const signals = score.signalsUnavailable;
 ...
 */
export function isDecimalUnits(amount) {
 ...
 */
export function decisionResponseDefect(read) {
    if (read.status === null)
        return "unreadable";
    if (read.status === 404 && isPlainObject(read.body) && read.body.error === "not_found")
        return "uncatalogued";
    if (read.status < 200 || read.status >= 300)
        return "unreadable";
    if (!isPlainObject(read.body))
        return "unreadable";
    return null;
}
